...
import { emploiDuTemps, classes, matieres, enseignants } from '../../data/mockData';
...
export default function EmploiDuTempsPage() {
  ...
  const getEvent = (jour: string, heure: string) => classSchedule.find(e => e.jour === jour && e.heure === heure);
  ...
  const handleDelete = (id: string, matiereName: string) => {
    if (window.confirm(`Voulez-vous vraiment supprimer le cours de ${matiereName} ?`)) {
      ...
      showToast(`Cours de ${matiereName} supprimé.`, 'success');
    }
  };
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    if (existing) {
      showToast(`Ce créneau est déjà occupé par un cours de ${existing.matiere}.`, 'warning');
      return;
    }
    ...
    showToast(`Cours de ${mat.nom} planifié avec succès.`, 'success');
  ...
  return (
    <div className="fade-in">
      <div className="page-header">
        <div>
          <div className="breadcrumb"><span>Académique</span><span className="breadcrumb-sep">/</span><span>Emploi du temps</span></div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div className="tab-bar" style={{ border: 'none', marginBottom: 0 }}>
            <div className={`tab-item ${vue === 'semaine' ? 'active' : ''}`} onClick={() => setVue('semaine')}>Semaine</div>
            <div className={`tab-item ${vue === 'jour' ? 'active' : ''}`} onClick={() => setVue('jour')}>Jour</div>
          </div>
        </div>
      ...
      <div className="grid-3 mt-24">
        ...
        <div className="card">
          <div className="card-body" style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '12px', color: '#6b7280' }}>Matières</div>
          </div>
        </div>
        <div className="card">
          <div className="card-body" style={{ textAlign: 'center' }}>
